import { Controller, Post, Body, Delete, ValidationPipe, Inject } from '@nestjs/common';
...
export class UserClassController {
  ...
  async subsribe(@Body(ValidationPipe) subscribeUserDTO: SubscribeUserDTO): Promise<void> {
    ...
    if(!findUser){
      throw new NotFoundException(`There is not register a user with id: ${userId}`)
    }

    if(!findClass){
      throw new NotFoundException(`There is not a created class with id: ${classId}`)
    }
