...
export class TaskLanguageController {
  ...
  async createRelation( @Body(ValidationPipe) taskLanguageDTO: TaskLanguageDTO ): Promise<void> {
    ...
    const findLanguage = await this.programmingLanguageService.findById(
      taskLanguageDTO.languageId,
    );

    if (!findTask) {
      throw new NotFoundException(
        `There is not a task with the id: ${taskLanguageDTO.taskId}`,
      );
    }

    if (!findLanguage) {
      throw new NotFoundException(
        `There is not a language with the id: ${taskLanguageDTO.languageId}`,
      );
    }

    this.taskLanguageService.persist(
  ...
  async removeRelation( @Body(ValidationPipe) taskLanguageDTO: TaskLanguageDTO ): Promise<void> {
    ...
    if (!findEntity) {
      throw new NotFoundException(
        `There is not a asignation that involves a task id: ${taskLanguageDTO.taskId} and language id: ${taskLanguageDTO.languageId} `,
      );
    }
