import { Controller, BadRequestException, Get, Post, Put, Delete,Query, Param, Body} from "@nestjs/common";
...
export class DeliveryCommentController{
    constructor(private deliveryCommentService:DeliveryCommentService,private taskDeliveryService:TaskDeliveryService){}

    @Get()
    async findAllByDeliveryId(@Query("deliveryId") deliveryId:number):Promise<DeliveryCommentVO[]>{
        if(typeof(deliveryId)==='undefined'){
            throw new BadRequestException(`you must provide the query parameter : 'deliveryId' `);
        }
        const findTaskDelivery= await this.taskDeliveryService.findById(deliveryId);
        
        if(!findTaskDelivery){
            throw new NotFoundException(`There is not a task delivery with the id: ${deliveryId}`)
        }
